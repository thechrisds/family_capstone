...
<div class="inner-book">
  ...
  <div class="img final-page" style="padding-top: calc(1.07 * 100%)">
    ...
    />
          ...
          <h3 class="tb-book-author">{{ book.author_name[0] }}</h3>
      </div>
      <b-button v-on:click="addBook"> Add Book </b-button>
  </div>
</template>
<script>
...
export default{
  ...
  methods: {
      ...
      addBook(){
        this.bookToAdd.bookTitle = this.books[0].title;
        this.bookToAdd.bookAuthor = this.books[0].author_name[0];
        this.bookToAdd.isbn = this.isbn;
        console.log(this.bookToAdd);
        bookService.addBooks(this.bookToAdd).then(response =>{
          console.log(response)
